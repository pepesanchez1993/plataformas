...
import { fileToThumbDataURL } from '../lib/image.js';
...
function machineForm(existing, onSaved) {
  ...
  // --- Foto de la máquina (cámara/galería del móvil, guardada en el dispositivo) ---
  let photoData = m.photo || '';
  const previewImg = el('img', { class: 'photo-preview', alt: 'Foto de la máquina' });
  const fileInput = el('input', {
    type: 'file', accept: 'image/*', capture: 'environment', class: 'photo-file',
  });
  const removeBtn = el('button', { type: 'button', class: 'btn btn--ghost btn--xs' }, 'Quitar foto');
  const pickLabel = el('label', { class: 'btn btn--ghost btn--sm photo-pick' }, ['📷 Foto de la máquina', fileInput]);
  const photoRow = el('div', { class: 'photo-row' }, [previewImg, pickLabel, removeBtn]);
  const syncPhoto = () => {
    if (photoData) { previewImg.src = photoData; previewImg.style.display = ''; removeBtn.style.display = ''; }
    else { previewImg.removeAttribute('src'); previewImg.style.display = 'none'; removeBtn.style.display = 'none'; }
  };
  syncPhoto();
  fileInput.addEventListener('change', async () => {
    ...
    if (!file) return;
    try {
      photoData = await fileToThumbDataURL(file);
      syncPhoto();
    } catch (err) {
      toast(err.message || 'No se pudo cargar la foto', 'err');
    } finally {
      fileInput.value = '';
    }
  });
  removeBtn.addEventListener('click', () => { photoData = ''; syncPhoto(); });

  const form = el('form', { class: 'form' }, [
    fName.wrap, fZone.wrap, fSource.wrap, fClub.wrap, fNotes.wrap, fCardio, photoRow,
...
function machineCard(m, rerender) {
  ...
  const thumb = m.photo
    ? el('img', { class: 'card-item__photo', src: m.photo, alt: m.name, loading: 'lazy' })
