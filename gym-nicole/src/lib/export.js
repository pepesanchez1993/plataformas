...
export async function exportJSON() {
  ...
  download(`gym-nicole-backup-${stamp()}.json`, JSON.stringify(dump, null, 2));
...
export async function exportLogsCSV(machinesById) {
  ...
  download(`gym-nicole-logs-${stamp()}.csv`, '﻿' + [head.join(';'), ...rows].join('\n'), 'text/csv');
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      try {
        ...
        if (dump.app && dump.app !== 'gym-nicole') {
          throw new Error('Este backup no es de Gym Nicole.');
        }
